...
Follow these conventions when adding skills to the repository tree:
- **Frontmatter**: Must start at byte 0 with `---`, include `name`, `description` (≤1024 chars), and `metadata.hermes`.
- **Structure**: `# Title` $\rightarrow$ `## Overview` $\rightarrow$ `## When to Use` $\rightarrow$ Body $\rightarrow$ `## Common Pitfalls` $\rightarrow$ `## Verification Checklist`.
- **Limits**: Total file size ≤ 100k chars (aim for 8-15k).
- **Validation**: Use `tools/skill_manager_tool.py` for frontmatter validation.

### Container Supervision (s6-overlay)
Hermes uses s6-overlay for Docker supervision:
- **Architecture**: `/init` (PID 1) $\rightarrow$ `main-wrapper.sh` (CMD).
- **Profile Gateways**: Runtime-registered under `/run/service/gateway-<name>`.
- **Recipes**: Use `/command/s6-svstat` to inspect services.
- **Pitfalls**: `docker exec` defaults to root; always use `--user hermes` or absolute paths to `/command/`.

### TUI Slash Command Debugging
Slash commands span Python (registry), Gateway (bridge), and Ink/TS (frontend):
- **Investigation**: Check `hermes_cli/commands.py` (truth) $\rightarrow$ `tui_gateway/server.py` $\rightarrow$ `ui-tui/src/app/slash/`.
- **Autocomplete**: Driven by `COMMAND_REGISTRY` in Python. If missing, add `CommandDef` entry.
- **Verification**: Rebuild TUI (`npm --prefix ui-tui run build`) before testing.
